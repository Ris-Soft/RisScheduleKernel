...
import { configType, lessonTaget, subjectTagret, scheduleType, temporarySchedule } from "./types";
...
export class scheduleKernel {
  ...
  getTeacherLessons(teacherName: string) {
    const teacherSubjects = this.config.subjects.filter(subject => 
      subject.teacherName === teacherName
    );

    return teacherSubjects.map(subject => {
      const lessons = this.config.schedules.flatMap(schedule => 
        schedule.lessons
          .map((lesson, index) => ({
  ...
  getSubjectLessons(subjectUuid: UUID) {
    return this.config.schedules.flatMap(schedule => 
      schedule.lessons
        .map((lesson, index) => ({
          dayIndex: schedule.activeDay || 0,
          lessonIndex: index,
   ...
   */
  editLesson(dayIndex: 1 | 2 | 3 | 4 | 5 | 6 | 7, lessonIndex: number, subjectUuid: UUID, week?: "odd" | "even"): boolean {
    ...
    const schedule = this.config.schedules.find(s => 
      !s.dateMode && 
  ...
  deleteSubject(uuid: UUID): boolean {
    ...
    // 检查是否有课程正在使用此科目
    const isInUse = this.config.schedules.some(schedule =>
      schedule.lessons.some(lesson => lesson.subjectUuid === uuid)
    );
  ...
  private getScheduleForDate(date: Date): scheduleType | undefined {
    ...
    // 首先查找是否存在临时课程安排
    const temporarySchedule = this.config.temporarySchedules?.find(ts => 
      new Date(ts.date).getTime() === date.getTime()
    );
